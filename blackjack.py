...
def simulate_one_step(state, action, userCard, dealCard, ccards, stand):
    ...
    userA = state[1]
    dealSum = 0
    for i in range(len(dealCard)):
        dealSum += getAmt(dealCard[i])
    dealA = state[3]
    dealFirst = state[2]
    dealAFirst = state[3]
    stand = False
    ...
    # check if given state is gameover, return none
    if dealSum >= 17:
        gameover = True
    else:
        gameover = False
    if len(userCard) == 2 and userSum == 21 and dealSum >= 17:
        gameover = True
    ...
    if gameover == True:
        ...
        if (userSum <= 21 and dealSum < userSum) or dealSum > 21:
            reward = 1
        elif (userSum > 21 and userA == 0) or dealSum <= 21 and dealSum > userSum:
            reward = -1
        elif stand == True and dealSum > userSum:
            reward = -1
        return (None, reward)
    ...
    if not (gameover or stand) and action == 0:
        ...
        if userSum > 21:
            bust = True
    if (bust == True) or (not gameover and action == 1):
        # Dealer plays, user stands
        stand = True
        if dealSum == 21:
            pass
...
def reward_to_go(s, gamma, episode):
    ...
    # get index from episodes list
    k = episode.index(s)

    for i in range(k, len(episode)):
        ret += (gamma**(i-k))*episode[i][1]
    return ret
    ...
    for i in range(50):
        ...
        while state is not None:
            ...
            if policy(state[0]) == 0:
                ...
                stand = False
            next_s = simulate_one_step(
                state, policy(state[0]), userCard, dealCard, ccards, stand)
            if (next_s[0] is not None):
                TDvalues[state] = TDvalues[state] + alpha * \
                    (next_s[1]+gamma*TDvalues[next_s[0]]-TDvalues[state])
            else:
                TDvalues[state] = TDvalues[state] + alpha * \
                    (next_s[1]+gamma*0-TDvalues[state])
            state = next_s[0]


def pick_actions(s, eps, Qvalues):
    if random.random(0, 1) < eps:
...
def Q_Learning(states, gamma, Qvalues, NQ):
    for i in range(50):
        ...
        userSum, userA, dealSum, dealA, dealFirst, dealAFirst = initGame(
            ccards, userCard, dealCard)
        # create arbitrary state
        s = make_state(userSum, userA, dealFirst, dealAFirst)
        eps = 0.1
        while s is not None:
            a = pick_actions(s, eps, Qvalues)
            NQ[s] += 1
            alpha = 10/9+NQ[s]
            next_s = simulate_one_step(
                s, a, userCard, dealCard, ccards, True if a == 1 else False)
            if next_s[0] is not None:
                Qvalues[s][a] = Qvalues[s][a] + alpha * \
                    max(Q[next_s][a]-Qvalues[s][a])
            # else:
    pass
                ...
                if dealSum == 21:
                    ...
                    while dealSum <= userSum and dealSum < 17:
                        ...
                        while dealSum > 21 and dealA > 0:
                            ...
                            dealSum -= 10
        state = make_state(userSum, userA, dealFirst, dealAFirst)
        MCU = font.render('MC-Utility of Current State: %f' %
                          MCvalues[state], 1, black)
        TDU = font.render('TD-Utility of Current State: %f' %
                          TDvalues[state], 1, black)
        QV = font.render('Q values: (Hit) %f (Stand) %f' %
                         (Qvalues[state][0], Qvalues[state][1]), 1, black)
